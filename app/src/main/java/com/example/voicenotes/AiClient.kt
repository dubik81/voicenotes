...
object AiClient {
    ...
    private fun allVariantsPrompt(): String {
        ...
        sb.append("Разрешено ТОЛЬКО: исправить ошибки распознавания (окончания, падежи, созвучные ")
        sb.append("слова), убрать чистые слова-паразиты («э-э», «ну», «значит», «короче», «это самое», ")
        sb.append("«как бы», «типа»), убрать повторы-заики, расставить пунктуацию и заглавные. ")
        sb.append("СТРОГО ЗАПРЕЩЕНО удалять ЛЮБЫЕ содержательные фразы, даже служебные или ")
        sb.append("переходные («конец стихотворения», «следующий пункт», «это всё», «конец» и т.п.) — ")
        sb.append("сохраняй их ВСЕ. НЕ решай сам, что важно, а что нет. НЕ сокращай, НЕ пересказывай. ")
        sb.append("Количество предложений в ответе = количеству в оригинале (кроме склеенных обрывков ")
        sb.append("одной фразы). Итог: тот же текст, та же длина, только грамотно оформленный.\n")

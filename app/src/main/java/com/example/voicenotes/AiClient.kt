...
object AiClient {
    ...
    private val CLEAN_RULE =
        ...
        "искажённые окончания и предлоги. ЗНАКОВ ПРЕПИНАНИЯ И ЗАГЛАВНЫХ ВО ВХОДЕ НЕТ ВООБЩЕ — " +
        "они намеренно убраны, чтобы ты расставил их сам, по смыслу звучащей речи.\n" +
        "ЗАДАЧА: восстановить, что человек произнёс на самом деле, и оформить это как текст: " +
        "разделить на предложения ПО СМЫСЛУ, расставить точки, запятые и заглавные буквы.\n" +
        "КАК: читай поток слов как звучащую речь; если слово бессмысленно в контексте — подбери созвучное, " +
        ...
        "ставь по смыслу речи — во входе их нет, разбивка на предложения целиком за тобой. Убирай только " +
    ...
    private fun request(
        ...
        // Суточную квоту минутной паузой не переждать: держим блокировку дольше, чтобы
        // приложение не долбило сервер и не показывало человеку ложную надежду.
        fun noteRateLimit(msg: String) {
            val daily = msg.lowercase().let { it.contains("суточн") || it.contains("per-day") || it.contains("daily") }
            rateLimitedUntil = System.currentTimeMillis() + if (daily) 30 * 60_000L else 60_000L
            Diagnostics.error("Облако заблокировано на ${if (daily) "30 мин (суточная квота)" else "минуту (частые запросы)"}")
        }
        ...
        repeat(2) {
            if (left() > 5_000) {
                try {
                    ...
                    if (lastError.contains("429")) { noteRateLimit(lastError); throw RuntimeException(lastError) }
    ...
    private fun explainError(code: Int, response: String): String {
        ...
        return when (code) {
            ...
            // v130. Раньше всегда писали «лимит 20 запросов в минуту, подождите минуту».
            // В тесте пользователя 429 держался с 17:55 до 19:01 — то есть это был не
            // минутный лимит, а СУТОЧНАЯ квота бесплатных моделей. Человек ждал минуту,
            // пробовал снова, снова получал отказ и решал, что приложение сломано.
            // Теперь смотрим, что именно ответил сервер, и говорим правду.
            429 -> {
                val m = serverMsg.lowercase()
                when {
                    m.contains("per-day") || m.contains("per day") || m.contains("daily") ->
                        "Исчерпан СУТОЧНЫЙ лимит бесплатных запросов OpenRouter. " +
                        "До завтра облако недоступно — работайте в режиме «Смысл Офл» " +
                        "или пополните баланс OpenRouter."
                    m.contains("per-minute") || m.contains("per minute") ->
                        "Слишком часто: лимит запросов в минуту. Подождите минуту."
                    else ->
                        "Лимит бесплатных запросов OpenRouter (429). Если повторяется дольше " +
                        "минуты — вероятно, исчерпана суточная квота: до завтра работайте в «Смысл Офл»." +
                        if (serverMsg.isNotBlank()) " [$serverMsg]" else ""
                }
            }
    ...
    private fun systemPrompt(level: Level, tone: Tone): String {
        val task = when (level) {
            ...
            Level.BRIEF ->
                ...
                "ОБЯЗАТЕЛЬНО сохраняй отрицания: если в тексте «не», «нельзя», «невозможно» — " +
                "они должны остаться. Менять утверждение на противоположное запрещено " +
                "(нельзя «это не означает, что системы автономны» превращать в «системы автономны»). " +
            ...
            Level.GIST ->
                ...
                "ОБЯЗАТЕЛЬНО сохраняй отрицания («не», «нельзя», «невозможно») — переворачивать " +
                "смысл на противоположный запрещено. Без нумерованных списков и разметки. " +

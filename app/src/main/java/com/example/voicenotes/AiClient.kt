...
object AiClient {
    ...
    private fun allVariantsPrompt(): String {
        ...
        sb.append("- CLEAN («Чисто»): ЭТО НЕ СЖАТИЕ И НЕ РЕДАКТУРА. Верни ВЕСЬ текст целиком. ")
        sb.append("ВАЖНО: исходный текст — это РАСПОЗНАННАЯ ПРОГРАММОЙ речь. Пунктуация и заглавные ")
        sb.append("в нём расставлены автоматически по правилам (без ИИ и человека) и ЧАСТО ОШИБОЧНЫ — ")
        sb.append("НЕ доверяй им, переосмысли пунктуацию и границы предложений ПО СМЫСЛУ сказанного. ")
        sb.append("Восстанавливай то, что реально сказал человек, учитывая типичные ошибки распознавания речи. ")
        sb.append("Разрешено: исправить ошибки распознавания (окончания, падежи, созвучные слова), ")
        sb.append("убрать слова-паразиты («э-э», «ну», «значит», «короче», «это самое», «как бы», «типа»), ")
        sb.append("убрать повторы-заики, расставить ПРАВИЛЬНУЮ пунктуацию и заглавные по смыслу. ")
        sb.append("СТРОГО ЗАПРЕЩЕНО удалять содержательные фразы, даже служебные/переходные ")
        sb.append("(«конец стихотворения», «следующий пункт», «это всё») — сохраняй их ВСЕ. ")
        sb.append("НЕ решай сам что важно, НЕ сокращай, НЕ пересказывай. Число предложений ПО СМЫСЛУ ")
        sb.append("сохрани. Итог: тот же текст той же длины, но грамотно оформленный по смыслу речи.\n")
        sb.append("- BRIEF («Кратко»): перескажи главное. НЕ заменяй конкретные факты обобщениями ")
        sb.append("(«замечательная погода» нельзя менять на «красиво» — это разный смысл). Длина ~40-50%.\n")
        sb.append("- GIST («Суть»): самая суть, 1-3 предложения, БЕЗ искажения смысла. Длина ~15-25%.\n\n")

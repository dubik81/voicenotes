...
object LocalAiEngine {
    ...
    @Volatile var leaksCaught: Int = 0
        private set
    // Ноль, пока утечек не было. Как только счётчик уходит вперёд — включается
    // безопасный режим: полная перезагрузка модели перед каждым вызовом.
    @Volatile private var leaksAtLastReload: Int = 0
          ...
          genMutex.withLock {
            ...
            try {
                ...
                if (noteId != lastGenNote) {
                    ...
                    lastGenNote = noteId
                } else if (leaksCaught > leaksAtLastReload) {
                    // v135: сторож ловил утечку 4 раза за один тест — значит resetContext
                    // между вызовами НЕ надёжен. Как только утечка случилась хотя бы раз,
                    // переходим на полную перезагрузку модели перед КАЖДЫМ вызовом до конца
                    // сессии. Медленнее, зато чужой текст в заметку не попадёт.
                    releaseCurrent()
                    Diagnostics.info("Перезагрузка модели: после утечки работаем в безопасном режиме")
